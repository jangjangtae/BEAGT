...
if __name__ == "__main__":
    ...
    try:
        old_net.load_state_dict(torch.load('./best_model_BEAGT'))
        old_reward_mean = evaluate_model_with_bugs(env, old_net)
        print(f"기존 model_BEAGT의 평균 보상: {old_reward_mean:.1f}")
    except FileNotFoundError:   
        print("기존 model_BEAGT가 없음, 기준 없이 새 모델 저장")
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        if iter_no == MAX_ITER:
            print("Training ends")
            print("\    /\ \n )  ( ')  meow!\n(  /  )\n \(__)|")
            torch.save(net.state_dict(), './model_BEAGT')

            final_reward_mean = evaluate_model_with_bugs(env, net)
            print(final_reward_mean)

            if final_reward_mean > old_reward_mean:
                torch.save(net.state_dict(), './best_model_BEAGT')
                print(" 기존 best_model_BEAGT보다 성능이 좋아서 업데이트됨!")
            else:
                print(" 기존 best_model_BEAGT가 더 우수하여 업데이트하지 않음.")
            break
